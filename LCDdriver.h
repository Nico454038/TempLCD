...
ground = pin 3 Vo grounded for highest contrast
...
void LCD_Init() {
P1SEL0 &= ~0x01;
P1SEL1 &= ~0x01; // configure upper nibble of P4 as GPIO
P1DIR |= 0x01;
P4SEL0 &= ~0xF0;
...
SysTick_Init(75000); // Volume 1 Program 4.7, Volume 2 Program 2.12
LCD_E = 0;
LCD_RS = 0; // E=0, R/W=0, RS=0
OutPort6();
...
// Inputs: letter is ASCII character, 0 to 0x7F (127)
// Outputs: none
...
n = number % 10;
LCD_OutCmd(0x80 + 0x44);
LCD_OutChar(n + '0');
number = number / 10;
n = number % 10;
LCD_OutCmd(0x80 + 0x43);
LCD_OutChar(n + '0');
number = number / 10;
n = number % 10;
LCD_OutCmd(0x80 + 0x42);
LCD_OutChar(n + '0');
number = number / 10;
LCD_OutCmd(0x80 + 0x41);
LCD_OutChar('.');
n = number % 10;
LCD_OutCmd(0x80 + 0x40);
LCD_OutChar(n + '0');
}
}
